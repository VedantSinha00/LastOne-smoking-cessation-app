...
## Feature Specification — Supporting Document for Logging & Streak System V1.2
...
- [[LastOne_Product_Foundations_V1]] — all five foundations touched; Stage System triggers this feature
- [[LastOne_Logging_System_Spec]] — primary spec this document extends
- [[LastOne_Streak_System_Spec_V1_2]] — relapse variables (rolling_14d_slips) trigger this system
...
4. **Professional resource tap rate** — % of users who tap through to a professional resource card when it is surfaced.
...
**Reads from:** rolling_14d_slips, slip_type (Logging Spec B2, Flow C), daily_checkin_satisfied, craving and overcome log counts, sos_uses_last_72h, support_person_configured, current_stage.

**Writes to:** giving_up_experience_triggered, giving_up_experience_outcome, support_person_configured (set during onboarding), support_person_briefing_sent.

**Requires:** Craving and overcome log history to populate Beat 2 data, contact picker access (device) for support person setup, WhatsApp/SMS deep link capability for briefing message send.
...
**Rejected:** A dedicated button or menu item for crisis support.
...
**Rejected:** Using the Tier 1 experience as a gateway to the restart flow.
...
**Rejected:** Showing overall quit progress, days smoke-free, or money saved.
...
**Rejected:** Routing to the Quit Group SOS ping or Quit Group chat.
...
**Rejected:** Building a separate "distress score" system with its own tracking.
...
**Rejected:** A dedicated "Get Help" or "Crisis Support" section.
...
**Rejected:** "Call My Person" as a bare one-tap dial feature with no context layer.
...
**Rejected:** Treating resource cards and the home screen trigger card as low-sensitivity (neutral-warm, single version).
...
After call ends / user returns to app: → GU-7 (post-call log, optional)
...
> ⚠️ **Resource Vetting Note — Action Required Before Implementation:** The specific organisations, phone numbers, and availability details in these cards must be verified by the team before being coded into the application. The cards below are structural templates. Do not ship with placeholder or unverified contact information.
...
**Actions:**
- Tap "Pick a contact" → Device contact picker → returns selected contact name + number → GU-10
...
**What the user sees:** The generated briefing message, pre-written, displayed in a text-area style view. An edit option. Two send buttons: "Send via WhatsApp" and "Send via SMS."

> **Briefing Message — Draft Copy (Operational, Not Finalised):** The following is a working draft of the briefing message. It is functional and can ship in this form if the content team has not yet reviewed it. It should be reviewed and refined before the production release.
...
All high-sensitivity copy is provided in three voice variants: **Steady & Direct**, **Emotional & Understanding**, **Light & Honest**. The user's chosen voice style (set in onboarding) determines which variant they see.
...
| Emotional & Understanding | "The last few days haven't been easy. We're still here. Take 2 minutes with us?" |
...
| Emotional & Understanding | "A difficult stretch doesn't mean something is wrong with you. It means quitting is genuinely hard — and you're still here." |
...
**Data line (all variants — only the framing copy varies):** "In the last 14 days, you resisted [X] cravings."
...
| Emotional & Understanding | "It's easy to remember the moments that were hard. These [X] moments — you got through them. Every single one." |
...
| Emotional & Understanding | "There's no pressure here. What would feel right for you in the next little while?" |
...
| Emotional & Understanding | "You don't have to sit with this alone." |
...
| Emotional & Understanding | "[Name] is there. You don't have to explain everything — just reach out." |
...
| Emotional & Understanding | "There are people whose job is to sit with this — and they've heard it before, without judgement." |
...
| Emotional & Understanding | "Sometimes the hardest part is asking for help. Let's make that easier. Pick one person, and we'll give them a heads-up on how to show up for you." |
...
**Device-only (SecureStore — never synced to server, T-F / F-1):**

| Field | Type | Required | Notes |
|---|---|---|---|
| support_person_name | string | No | Set during GU-9. Same single contact as Settings `sos_contact_name` and Coping Tools SOS contact. |
| support_person_number | string | No | Set during GU-9. E.164. Device-only — third-party PII, never on server. Lost on reinstall → re-entered. |

**Derived (not stored):**

| Field | Type | Notes |
|---|---|---|
| support_person_configured | boolean | Derived at read time: true if device-stored name + number are both present. Default false. |

> **Privacy note (T-F / F-1):** the support person's number and name are stored on-device only (SecureStore), never synced to the server. The app requires the number only to initiate the dial / WhatsApp deep link intent. This is the canonical reference position — Settings (`sos_contact_phone`) and Coping Tools (`user_sos_state`) defer to it; all three are the same one contact.
...
ELSE:
  resistance_count_fallback = COUNT of all overcome logs since quit_date
...
This is distinct from the original "Tough day. Want to chat with the AI coach?" nudge in the logging spec — that line routes to GU-8 (or GU-5 if support person is configured) now that the AI coach is not being implemented in V1.
...
- Fallback: GET count of all overcome logs since quit_date if 14-day count is zero.

---

*LastOne — Giving Up Support System Spec V1.0 | Supporting document for Logging & Streak System V1.2 | April 2026*
